...
public:
    ...
    virtual double Evaluate(const FormulaAST::Functor& get_value) const = 0;
...
public:
    ...
    double Evaluate([[maybe_unused]] const FormulaAST::Functor& get_value) const override {
        double answer;
        switch (type_) {
            case Add:
                answer = lhs_->Evaluate(get_value) + rhs_->Evaluate(get_value);
...
public:
    ...
    double Evaluate([[maybe_unused]] const FormulaAST::Functor& get_value) const override {
        return type_ == UnaryPlus ? operand_->Evaluate(get_value) : -1 * operand_->Evaluate(get_value);
    }
...
public:
    ...
    double Evaluate(const FormulaAST::Functor& get_value) const override {
...
double FormulaAST::Execute(const Functor& get_value) const {
